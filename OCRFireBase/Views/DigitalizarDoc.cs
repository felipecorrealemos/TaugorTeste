...
namespace OCRFireBase.Views
{
    public partial class DigitalizarDoc : Form
    {
        ...
        private void DigitalizarDoc_Load(object sender, EventArgs e)
        {
            ...
            comboBox2.Items.Add("Portugues Brasil");
        ...
        private async void EscaneandoDocs()
        {
            ...
            try
            {
                ...
                string pdfPath = Path.Combine(Directory.GetCurrentDirectory(), "temp\\file.pdf"); ;
                string pdfOutput = Path.Combine(Directory.GetCurrentDirectory(), "temp\\output.pdf");
                ...
                foreach (IDeviceInfo deviceInfo in manager.DeviceInfos)
                {
                    if (deviceInfo.Type == WiaDeviceType.ScannerDeviceType)
                    {
                        ...
                        if (String.Compare(deviceInfo.Properties["Name"].get_Value().ToString(), comboBox1.Text, StringComparison.OrdinalIgnoreCase) == 0)
                        {
                            ...
                            {
                                // Cria documento PDF
                                string tempFilePath = Path.Combine(Directory.GetCurrentDirectory(), "temp\\file123.jpeg");
                ...
                if (checkBox2.Checked)
                {
                    ...
                    await Task.Run(() => OCR.ScanDocOCR(idioma));
                    //IniciarServicoORC(idioma);
        ...
        {
            await SalvaPDF("Scan", originalImage, rotatedImage, checkBox1, @"temp\file.pdf");
        ...
        {
            await SalvaPDF("ScanOCR", originalImageOCR, rotatedImageOCR, checkBox1, @"temp\output.pdf");
        ...
        private async static Task SalvaPDF(string nomeArq, Bitmap originalImage, Bitmap rotatedImage, CheckBox checkBox, string arqTemp)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    //FileInfo fileInfo = new FileInfo(Path.Combine(Directory.GetCurrentDirectory(), @"temp\output.pdf"));
                    FileInfo fileInfo = new FileInfo(Path.Combine(Directory.GetCurrentDirectory(), arqTemp));
        ...
        private string VerificaIdioma()
        {
            ...
            if (comboBox2.Text == "Portugues Brasil")
            {
                return "por";
            }
