...
namespace OCRFireBase.Views
{
    public partial class ArquivosNuvem : Form
    {
        ...
        private void ApagarThumbsExistentes()
        {
            try
            {
                string appDataPathThumb = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "OCRFireBase");
                appDataPathThumb = Path.Combine(appDataPathThumb, "thumbnail");

                string[] arquivos = Directory.GetFiles(appDataPathThumb);
        ...
        private async void DownloadThumbs()
        {
            try
            {
                ...
                foreach (var storageObject in objects)
                {
                    ...
                    {
                        string novoArquivo = Path.Combine(appDataPathThumb, $"{storageObject.Name}");
                ...
                string[] thumbs = Directory.GetFiles(appDataPathThumb);
                int pos = 0;
            ...
            {
                MessageBox.Show("Não foi possivel carregar as thumbnails");
            }
        }
